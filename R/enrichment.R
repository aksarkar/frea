...
plot_enhancer_enrichments <- function(filename, cluster_density, plot_log_fold=FALSE, flip=FALSE) {
    ...
    my_gtable <- gtable:::rbind.gtable(ggplotGrob(enrichment_by_cluster(enrichments, cluster_density, scale_name)),
                                       ggplotGrob(density_by_cluster(cluster_density, keep=unique(enrichments$cluster))),
                                       size='last')
    ## Add tissue colors
